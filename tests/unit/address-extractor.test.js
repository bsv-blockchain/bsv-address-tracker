...
describe('AddressExtractor', () => {
  ...
  describe('createAddressFromHash160', () => {
    it('should create testnet address from hash160', () => {
      // hash160 from the test transaction: be83350213ab6483e111f675268b5bbaba7cdcae
      const hash160 = Buffer.from('be83350213ab6483e111f675268b5bbaba7cdcae', 'hex');
      const address = extractor.createAddressFromHash160(hash160, 'testnet');
      
      expect(address).toBe('mxtHrvoExpf55rts14HyyKeZc7FtwSoxY5');
    });

    it('should create mainnet address from hash160', () => {
      // Same hash160 but for mainnet should produce different address
      const hash160 = Buffer.from('be83350213ab6483e111f675268b5bbaba7cdcae', 'hex');
      const address = extractor.createAddressFromHash160(hash160, 'mainnet');
      
      // Should be different from testnet address
      expect(address).not.toBe('mxtHrvoExpf55rts14HyyKeZc7FtwSoxY5');
      // Should start with '1' for mainnet P2PKH addresses
      expect(address.charAt(0)).toBe('1');
    });
  });
});